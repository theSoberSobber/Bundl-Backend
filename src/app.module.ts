  ...
  onModuleInit() {
    try {
      ...
      const envVars = {
        ...
        REFRESH_TOKEN_SECRET: this.configService.getOrThrow(
          'REFRESH_TOKEN_SECRET',
        ),
        JWT_EXPIRES_IN: this.configService.getOrThrow('JWT_EXPIRES_IN'),

        // FCM
        FCM_SERVICE_FILE_PATH: this.configService.getOrThrow(
        ...
        // Firebase
        FIREBASE_SERVICE_ACCOUNT_PATH: this.configService.getOrThrow(
      ...
      console.error(
        'Application cannot start without all required environment variables',
      );
      process.exit(1);
    }
  }
