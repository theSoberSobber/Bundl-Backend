...
export class AuthService {
  ...
  constructor(
    ...
    this.isDebugMode = this.configService.get<string>('DEBUG_ENABLED') === 'true';
  ...
  async verifyOtpAndLoginOrCreateUser(tid: string, otp: string, fcmToken?: string): Promise<{ user: User; accessToken: string; refreshToken: string } | null> {
    // Get phone number from Redis
    const phoneNumber = await this.otpRedisService.getPhoneNumber(tid);
    
    if (!phoneNumber) {
      throw new BadRequestException('Session expired or invalid transaction ID');
    }
  ...
  private async storeTokenForUser(userId: string, token: string): Promise<void> {
    console.log(`Storing token for user: ${userId}, token: ${token.substring(0, 20)}...`);
  ...
  async refreshTokens(refreshToken: string): Promise<{ accessToken: string; refreshToken: string }> {
    try {
      ...
      if (isBlacklisted) {
        throw new ForbiddenException('Invalid refresh token, please sign in again');
      }
  ...
  async isTokenBlacklisted(token: string): Promise<boolean> {
    ...
    try {
      ...
      console.log(`Blacklist check result: ${exists ? 'Found in blacklist' : 'Not in blacklist'}`);
  ...
  private async blacklistAllUserTokens(userId: string): Promise<void> {
    ...
    try {
      ...
      console.log(`Found ${userTokens.length} tokens to blacklist for user ${userId}`);
  ...
  async signOut(userId: string): Promise<{ success: boolean }> {
    ...
    try {
      ...
      const userTokens = await this.redis.smembers(`user:${userId}:tokens`);
      console.log(`Found ${userTokens.length} active tokens for user ${userId}`);
